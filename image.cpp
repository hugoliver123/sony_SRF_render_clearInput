...
int pic[2460][3360];
void OB();
void WB(float G_reduce_ratio, float R_enhance_ratio, float B_reduce_ratio);
...
void rgb2bmp(char*);
void edge_enhance();
void increase_color_vibrancy(double contrast, double saturation);
...
int main(int argc, char* argv[])
{
	...
	{
		float G_reduce_ratio = 1;
		float B_reduce_ratio = 1;
		float R_enhance_ratio = 1;
		float colour_warm = 0;
		float init_colour_warm = 0; //这个变数存储最初的色温
		...
		OB();
		WB(G_reduce_ratio, R_enhance_ratio, B_reduce_ratio);
		color_interpolation();
		colormatrix();  //使用双线性插值法进行色彩插值
		median_filter();  //降低画面噪声
		gamma(1.06);
		edge_enhance();
		increase_color_vibrancy(15, 1.5);  //调整对比度和饱和度
		rgb2bmp(argv[k]);

		char output[256];
		FILE* pipe;
		char command[256];
		snprintf(command, sizeof(command), "python C:/Users/steve/Desktop/isGreen.py %s.bmp", argv[k]);  //请修改这里为python档案路径
		...
		if (_pclose(pipe) == -1) {
			perror("Error: Failed to close pipe");
		}
		colour_warm = atof(output);
		init_colour_warm = colour_warm;
		printf("colour warn: %f, G reduce ratio: %f, R enhance ratio: %f, B reduce ratio: %f\n \n", colour_warm, G_reduce_ratio, R_enhance_ratio, B_reduce_ratio);
		...
		fclose(fp);
			...
			if (colour_warm > 6720) {  //色温过高
				if (init_colour_warm < 6370) {
					break;
				} // 侦测预处理色温，防止死锁

				G_reduce_ratio -= 0.02;
				B_reduce_ratio -= 0.01;
				if (G_reduce_ratio <= 0.9 && R_enhance_ratio <= 1.020) {
					R_enhance_ratio += 0.005;
				}
			}
			else {  //色温过低
				if (init_colour_warm > 6720) {
					break;
				}// 侦测预处理色温，防止死锁

				G_reduce_ratio += 0.015;
				R_enhance_ratio -= 0.005;
			}
			
		} while ((colour_warm > 6720 || (colour_warm < 6610 && colour_warm > 6370)) && G_reduce_ratio > 0.60);
	}

	Beep(554.37, 20000);  //完成所以处理后发出提示音
...
void WB(float G_reduce_ratio, float R_enhance_ratio, float B_reduce_ratio)
{
	...
	if (counter != 0) {//find out the reference white point
		...
		G_factor = G_factor * G_reduce_ratio;
		R_factor = R_factor *  R_enhance_ratio;
		//B_factor = B_factor * B_reduce_ratio;
...
void color_interpolation()
{
	...
	for (i = 0; i < rows; i = i + 2) {
		for (j = 5; j < cols; j = j + 2) {
			r[i][j] = pic[i][j];
			g[i][j] = ((i - 1 >= 0 ? pic[i - 1][j] : 0) + (i + 1 < rows ? pic[i + 1][j] : 0) + (j - 1 >= 5 ? pic[i][j - 1] : 0) + (j + 1 < cols ? pic[i][j + 1] : 0)) / 4;
			b[i][j] = ((i - 1 >= 0 && j - 1 >= 5 ? pic[i - 1][j - 1] : 0) + (i - 1 >= 0 && j + 1 < cols ? pic[i - 1][j + 1] : 0) + (i + 1 < rows && j - 1 >= 5 ? pic[i + 1][j - 1] : 0) + (i + 1 < rows && j + 1 < cols ? pic[i + 1][j + 1] : 0)) / 4;
		}
	}
	...
	for (i = 1; i < rows; i = i + 2) {
		for (j = 5; j < cols; j = j + 2) {
			g[i][j] = pic[i][j];
			r[i][j] = ((i - 1 >= 0 ? pic[i - 1][j] : 0) + (i + 1 < rows ? pic[i + 1][j] : 0)) / 2;
			b[i][j] = ((j - 1 >= 5 ? pic[i][j - 1] : 0) + (j + 1 < cols ? pic[i][j + 1] : 0)) / 2;
		}
	}

	//E region
	for (i = 0; i < rows; i = i + 2) {
		for (j = 6; j < cols; j = j + 2) {
			b[i][j] = ((i - 1 >= 0 ? pic[i - 1][j] : 0) + (i + 1 < rows ? pic[i + 1][j] : 0)) / 2;
			g[i][j] = ((i - 1 >= 0 && j - 1 >= 6 ? pic[i - 1][j - 1] : 0) + (i - 1 >= 0 && j + 1 < cols ? pic[i - 1][j + 1] : 0) + (i + 1 < rows && j - 1 >= 6 ? pic[i + 1][j - 1] : 0) + (i + 1 < rows && j + 1 < cols ? pic[i + 1][j + 1] : 0) + (j - 1 >= 6 ? pic[i][j - 1] : 0) + (j + 1 < cols ? pic[i][j + 1] : 0)) / 6;
			r[i][j] = ((j - 1 >= 6 ? pic[i][j - 1] : 0) + (j + 1 < cols ? pic[i][j + 1] : 0)) / 2;
		}
	}
	//B region
	for (i = 1; i < rows; i = i + 2) {
		for (j = 6; j < cols; j = j + 2) {
			b[i][j] = pic[i][j];
			r[i][j] = ((i - 1 >= 0 && j - 1 >= 6 ? pic[i - 1][j - 1] : 0) + (i - 1 >= 0 && j + 1 < cols ? pic[i - 1][j + 1] : 0) + (i + 1 < rows && j - 1 >= 6 ? pic[i + 1][j - 1] : 0) + (i + 1 < rows && j + 1 < cols ? pic[i + 1][j + 1] : 0)) / 4;
			g[i][j] = ((j - 1 >= 6 ? pic[i][j - 1] : 0) + (j + 1 < cols ? pic[i][j + 1] : 0)) / 2;
		}
	}
...
//这里插入了一个新的方法， 主要用来对画面降噪，过对每个像素周围的3x3区域内像素值进行平均
void median_filter()
{
	int i, j;
	...
	for (i = 1; i < 2459; ++i) {
		for (j = 6; j < 3292; ++j) {
			...
			for (int m = -1; m <= 1; ++m) {
				for (int n = -1; n <= 1; ++n) {
					if (i + m >= 0 && i + m < 2460 && j + n >= 0 && j + n < 3294) {
						r_sum += r[i + m][j + n];
						g_sum += g[i + m][j + n];
						b_sum += b[i + m][j + n];
						count++;
...
// 这三个方法是用于色彩空间转换的，并非本人原创， 均来自网络整理
void rgb_to_hsl(int r, int g, int b, double* h, double* s, double* l)
{
	...
	double max = fmax(r_normalized, fmax(g_normalized, b_normalized));
	double min = fmin(r_normalized, fmin(g_normalized, b_normalized));
...
void hsl_to_rgb(double h, double s, double l, int* r, int* g, int* b)
{
	...
	else
	{
		...
		double r_normalized = hue_to_rgb(p, q, h + 1.0 / 3.0);
		double g_normalized = hue_to_rgb(p, q, h);
		double b_normalized = hue_to_rgb(p, q, h - 1.0 / 3.0);

		*r = (int)(r_normalized * 255.0);
		*g = (int)(g_normalized * 255.0);
		*b = (int)(b_normalized * 255.0);
	}
}



// 用于接受两个参数， 用于调整对比度和饱和度
void increase_color_vibrancy(double contrast, double saturation)
{
	int i, j;
	double r_contrast, g_contrast, b_contrast;

	// Calculate contrast adjustment factors
	r_contrast = (259.0 * (contrast + 255.0)) / (255.0 * (259.0 - contrast));
	g_contrast = (259.0 * (contrast + 255.0)) / (255.0 * (259.0 - contrast));
	b_contrast = (259.0 * (contrast + 255.0)) / (255.0 * (259.0 - contrast));
	...
	for (i = 0; i < 2460; i++)
	{
		...
		{
			// Adjust contrast
			r[i][j] = r_contrast * (r[i][j] - 128) + 128;
			g[i][j] = g_contrast * (g[i][j] - 128) + 128;
			b[i][j] = b_contrast * (b[i][j] - 128) + 128;
			...
			// Convert RGB to HSL
			double h, s, l;
			rgb_to_hsl(r[i][j], g[i][j], b[i][j], &h, &s, &l);
			...
			hsl_to_rgb(h, s, l, &r[i][j], &g[i][j], &b[i][j]);
		}
	}
}
